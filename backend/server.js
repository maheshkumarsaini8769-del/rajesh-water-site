...
var GITHUB_REPO = process.env.GITHUB_REPO || 'maheshkumarsaini8769-del/rajesh-water-site';
var GITHUB_BRANCH = process.env.GITHUB_BRANCH || 'main';
var GITHUB_TOKEN = process.env.GITHUB_TOKEN || '';

/* ---- GitHub API helpers (used on Vercel where filesystem is read-only) ---- */
function ghApi(method, urlPath, body) {
  return new Promise(function (resolve, reject) {
    ...
    var opts = {
      ...
      method: method,
      headers: {
        'User-Agent': 'rajesh-water-admin',
        'Authorization': 'token ' + GITHUB_TOKEN,
        'Accept': 'application/vnd.github.v3+json'
      }
    };
    if (data) opts.headers['Content-Type'] = 'application/json; charset=utf-8';
    ...
    var req = https.request(opts, function (res) {
      ...
      res.on('end', function () {
        ...
        catch (e) { resolve({ status: res.statusCode, body: { message: d } }); }
      });
    });
    req.on('error', reject);
...
function ghGetFile(filePath) {
  return ghApi('GET', '/repos/' + GITHUB_REPO + '/contents/' + filePath + '?ref=' + GITHUB_BRANCH);
}

function ghPutFile(filePath, contentBase64, message, sha) {
  var body = { message: message, content: contentBase64, branch: GITHUB_BRANCH };
  if (sha) body.sha = sha;
  return ghApi('PUT', '/repos/' + GITHUB_REPO + '/contents/' + filePath, body);
}

function ghUpdateFile(filePath, newContent, message) {
  return ghGetFile(filePath).then(function (res) {
    var sha = (res.status === 200 && res.body && res.body.sha) ? res.body.sha : null;
    return ghPutFile(filePath, Buffer.from(newContent).toString('base64'), message, sha);
  });
}
...
function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/site-data') {
    ...
    req.on('end', async function () {
      ...
      if (IS_VERCEL && GITHUB_TOKEN) {
        ...
        try {
          var ghRes = await ghUpdateFile('frontend/data/site-data.js', src, 'admin: update site-data');
          console.log('[server:gh] site-data updated:', ghRes.status);
        } catch (e) {
          console.error('[server:gh] site-data update failed:', e.message);
        }
      }
  ...
  if (req.method === 'POST' && pathname === '/api/deploy') {
    if (!requireAdmin(req, res)) return true;
    if (IS_VERCEL) {
      send(res, 200, { ok: true, output: 'Vercel: deploy triggered by GitHub push (auto)' });
      return true;
    }
  ...
  if (req.method === 'POST' && pathname === '/api/upload') {
    ...
    req.on('end', function () {
      ...
      var gitPath = 'frontend/images/' + fileName;

      if (IS_VERCEL) {
        if (!GITHUB_TOKEN) { send(res, 500, { ok: false, error: 'GITHUB_TOKEN not configured on Vercel' }); return; }
        ghPutFile(gitPath, b64, 'admin: upload ' + fileName).then(function (r) {
          if (r.status === 201 || r.status === 200) {
            console.log('[upload:gh] saved', fileName, '(' + buf.length + ' bytes)', '→', relPath);
            send(res, 200, { ok: true, path: relPath, fileName: fileName, size: buf.length });
          } else {
            console.error('[upload:gh] failed:', r.status, r.body && r.body.message);
            send(res, 500, { ok: false, error: 'GitHub API: ' + (r.body && r.body.message || r.status) });
          }
        }).catch(function (e) {
          console.error('[upload:gh] error:', e.message);
          send(res, 500, { ok: false, error: 'GitHub API error: ' + e.message });
        });
      } else {
        ...
        try {
          ...
          console.log('[upload:fs] saved', fileName, '(' + buf.length + ' bytes)', '→', relPath);
          send(res, 200, { ok: true, path: relPath, fileName: fileName, size: buf.length });
        } catch (e) {
          console.error('[upload:fs] write failed:', e.message);
