...
function readSiteDataJs() {
  if (MEM.siteData && Object.keys(MEM.siteData).length) return Promise.resolve(memCopy(MEM.siteData));
  if (db.state().on) {
    return db.loadSiteData().then(function (md) {
      if (md && Object.keys(md).length) { MEM.siteData = md; return memCopy(md); }
      return readSiteDataFromFile();
    }).catch(function () { return readSiteDataFromFile(); });
  }
  return Promise.resolve(readSiteDataFromFile());
}
function readSiteDataFromFile() {
  try {
...
function handleApi(req, res, pathname) {
  ...
  /* ---- Image upload: compresses and returns data URL (stored in MongoDB via site-data) ---- */
  if (req.method === 'POST' && pathname === '/api/upload') {
    ...
    req.on('end', function () {
      ...
      console.log('[upload] received image', buf.length, 'bytes');
      send(res, 200, { ok: true, dataUrl: 'data:image/webp;base64,' + b64, size: buf.length });
    });
    return true;
  }
